...
# %% scrolled=true
df=pd.DataFrame(animals, columns=['Name', 'speed', 'lifespan'])

print(df.lifespan.loc[2], type(df.lifespan.loc[2]))

print(df.Name.loc[2], type(df.Name.loc[2]))
#constat : les types sont coherents

df.loc[2,'Name']='Grey Elephant'
print(df.Name.loc[2], type(df.Name.loc[2]))
...
# %%
T=df.to_numpy()
print(type(T[0,0]), type(T[0,1]))
#un tableau numpy non homogène ...
type(T)
...
# %% tags=["level_intermediate"]
df4i=pd.concat([df1i.merge(df2i, left_index=True, right_on='name'),df3i])
df4i
