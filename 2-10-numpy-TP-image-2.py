...
def lire_fichier(filename):
    f=open(filename)
    dico={l.split()[0]:[int(i) for i in l.split()[1:]]  for l in f.readlines()}
    f.close()
    return dico
        
dico=lire_fichier('rgb-codes.txt')


# %%
def patchwork(liste_coul, dico):
    n=round(np.sqrt(len(liste_coul)))+1
    T=np.zeros((n**2,3),dtype='uint8')
    for t in range(len(liste_coul)):
        T[t]=dico[liste_coul[t]]
...
# %% {"scrolled": true}
# à base de blanc:
l=[i[0] for i in dico.items() if (255 in i[1])]
print(l)
P=patchwork(l,dico)
plt.imsave('patchwork.jpg', P)

# %%
M=[i[0] for i in dico.items()]
P=patchwork(M,dico)
plt.imshow(P);
